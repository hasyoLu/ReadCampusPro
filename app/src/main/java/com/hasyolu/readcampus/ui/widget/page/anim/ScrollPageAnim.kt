package com.hasyolu.readcampus.ui.widget.page.anim
...
import com.hasyolu.readcampus.ui.widget.page.model.BitmapView
import com.hasyolu.readcampus.ui.widget.page.PageAnimation
import com.hasyolu.readcampus.ui.widget.page.event.OnCurPageChangeListener
import com.hasyolu.readcampus.ui.widget.page.model.Direction
...
class ScrollPageAnim(
     ...
     */
    @Synchronized
    override fun startAnimBefore():Boolean {
        isRunning = true
        mScroller.fling(
            0,
            mTouchY.toInt(),
    ...
    private fun fillDown(bottomEdge: Int, offset: Int) {
        ...
        while (realEdge < mViewHeight && mActiveViews.size < 2) {
            ...
            if (!isRefresh) {
                val hasNext = mListenerCur.hasNext() //如果不成功则无法滑动
    ...
    private fun fillUp(topEdge: Int, offset: Int) {
        ...
        while (realEdge > 0 && mActiveViews.size < 2) {
            ...
            mNextBitmap = view.bitmap
            if (!isRefresh) {
                val hasPrev = mListenerCur.hasPrev() // 如果不成功则无法滑动
                // 如果不存在next,则进行还原
                if (!hasPrev) {
                    mNextBitmap = cancelBitmap
                    for (activeView in mActiveViews) {
     ...
     * @param paddingTop
     */
    private fun eraseBitmap(b: Bitmap, width: Int, height: Int, paddingLeft: Int, paddingTop: Int) {
