 ...
 * 仿真动画
 */
class SimulationPageAnim(w: Int, h: Int, view: View, listenerCur: OnCurPageChangeListener) :
    ...
    // 是否属于右上左下
    private var mIsRTandLB: Boolean = false
     ...
     * 设置方向扩展
     */
    override fun setDirectionExt(direction: Direction) {
        when (direction) {
            Direction.PRE -> {
                //上一页滑动不出现对角
     ...
     * 设置开始位置
     */
    override fun setStartPointExt(x: Float, y: Float) {
     ...
     * 设置触摸点
     */
    override fun setTouchPointExt(x: Float, y: Float) {
